...
namespace AMCL.DL
{
    public class CommonGateway
    {
        ...
        public int Insert(Hashtable hashTable, string SourceTable, OracleConnection Conn)
        {


            try
            {
                OracleCommand oracleComd = new OracleCommand("SELECT * FROM " + SourceTable + " WHERE NOT(1=1)", Conn);
                OracleDataAdapter ODP = new OracleDataAdapter(oracleComd);
                DataSet DS = new DataSet();
                ODP.Fill(DS, SourceTable);
                DataRow drAddrow = DS.Tables[0].NewRow();
