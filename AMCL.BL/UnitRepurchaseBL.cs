...
namespace AMCL.BL
{
    public class UnitRepurchaseBL
    {
        ...
        public void saveRepurchase(DataTable dtTransferSaleCert, UnitHolderRegistration regObj, UnitRepurchase repObj, UnitUser unitUserObj)
        {
            try
            {
                ...
                {                   
                    ...
                    htRepurchaseRegInfo.Add("REG_BK", regObj.FundCode.ToString().ToUpper());
                    htRepurchaseRegInfo.Add("REG_BR", regObj.BranchCode.ToString().ToUpper());
                    if (regObj.BranchCode.ToString().ToUpper() != "AMC/01")
                    {
                        if (repObj.PayType.ToString().ToUpper() == "EFT")
                        {
                            htRepurchaseRegInfo.Add("BEFTN_DATE", repObj.RepurchaseDate);
                        }
                        else if (repObj.PayType.ToString().ToUpper() == "CHQ")
                        {
                            htRepurchaseRegInfo.Add("CHEQUE_DATE", repObj.RepurchaseDate);
                            htRepurchaseRegInfo.Add("CHEQUE_NO", "BR_STATEMENT");
        ...
        public DataTable dtGetChequePaymentData(string filter)
        {
            StringBuilder sbQuery = new StringBuilder();
            sbQuery.Append(" SELECT A.VOUCHER_NO, FUND_INFO.FUND_NM,FUND_INFO.BANK_CODE, U_MASTER.HNAME,A.AUDITED_BY, A.REP_NO, A.REG_BK, A.REG_BR, A.REG_NO, A.REP_DT, A.REP_PRICE, A.QTY, A.TOTAL");
            sbQuery.Append(" FROM (SELECT VOUCHER_NO, AUDITED_BY,REP_NO, REG_BK, REG_BR, REG_NO, TO_CHAR(REP_DT, 'DD-MON-YYYY') AS REP_DT, REP_PRICE, SUM(QTY) AS QTY, SUM(REP_PRICE * QTY) AS TOTAL ");
            sbQuery.Append(" FROM REPURCHASE WHERE (PAY_TYPE = 'CHQ') AND (CHEQUE_DATE IS NULL)  AND (CHEQUE_NO IS NULL) ");
            sbQuery.Append(" GROUP BY VOUCHER_NO,AUDITED_BY, REP_NO, REG_BK, REG_BR, REG_NO, REP_DT, REP_PRICE) A ");
            sbQuery.Append(" INNER JOIN   U_MASTER ON A.REG_BK = U_MASTER.REG_BK AND A.REG_BR = U_MASTER.REG_BR AND A.REG_NO = U_MASTER.REG_NO INNER JOIN ");
            sbQuery.Append(" FUND_INFO ON A.REG_BK = FUND_INFO.FUND_CD  ");
            sbQuery.Append(" WHERE 1=1" + filter.ToString());
            ...
            DataTable dtChequeData = commonGatewayObj.Select(sbQuery.ToString());
            return dtChequeData;
        }
        ...
        public DataTable dtGetChequePaymentDataWithVoucherNo(string filter)
        {
            DataTable dtChequeData = dtGetChequePaymentData(filter);
            DataTable dtChequeDataTableWithVoucherNo = dtBEFTNTableWithVoucherNo();
            if (dtChequeData.Rows.Count > 0)
            {
                DataRow drDataTable;
                for (int loop = 0; loop < dtChequeData.Rows.Count; loop++)
                {
                    drDataTable = dtChequeDataTableWithVoucherNo.NewRow();
                    drDataTable["VOUCHER_NO"] = getNexAccountVoucherNo(GetaccountSchema(dtChequeData.Rows[loop]["REG_BK"].ToString()));
                    drDataTable["FUND_NM"] = dtChequeData.Rows[loop]["FUND_NM"].ToString();
                    drDataTable["REG_BK"] = dtChequeData.Rows[loop]["REG_BK"].ToString();
                    drDataTable["REG_BR"] = dtChequeData.Rows[loop]["REG_BR"].ToString();
                    drDataTable["REG_NO"] = dtChequeData.Rows[loop]["REG_NO"].ToString();
                    drDataTable["REP_NO"] = dtChequeData.Rows[loop]["REP_NO"].ToString();
                    drDataTable["REP_DT"] = dtChequeData.Rows[loop]["REP_DT"].ToString();
                    drDataTable["HNAME"] = dtChequeData.Rows[loop]["HNAME"].ToString();
                    drDataTable["QTY"] = dtChequeData.Rows[loop]["QTY"].ToString();
                    drDataTable["REP_PRICE"] = dtChequeData.Rows[loop]["REP_PRICE"].ToString();
                    drDataTable["TOTAL"] = dtChequeData.Rows[loop]["TOTAL"].ToString();

                    dtChequeDataTableWithVoucherNo.Rows.Add(drDataTable);
                }
            }
            return dtChequeDataTableWithVoucherNo;
        ...
        {
            StringBuilder sbQuery = new StringBuilder();
            sbQuery.Append(" SELECT A.BANK_AC_TYPE, A.BANK_AC_NO AS FUND_ACC_NO, A.FUND_NM || '(- A/C #' || A.BANK_AC_NO || '- ROUTING # ' || A.BANK_ROUTING_NO || ')' AS ACCOUNT_INFO, A.FUND_NM, A.BANK_ROUTING_NO,BANK_NAME.BANK_NAME,BANK_BRANCH.BRANCH_NAME, B.REG_BK,B.REG_BR,B.REG_NO,B.REP_NO,");
        ...
        public bool IsDuplicateAccVoucherNo(string accountSchema,string voucher_No,string voucher_type,string yearStartDate,string yearEndDate)
        {
          
            DataTable dtVoucher = new DataTable();
            dtVoucher = commonGatewayObj.Select("SELECT * FROM "+ accountSchema + ".GL_TRAN WHERE  VOUCHER_NO='" + voucher_No + "'AND VOUCHER_TYPE='" + voucher_type + "' AND  TRAN_DATE BETWEEN '"+ yearStartDate + "' AND '" + yearEndDate + "'");
        ...
        //SMS
        public bool IsValidMobileNumber(UnitHolderRegistration regObj)
        {
            bool validMobile = false;
            DataTable dtValidMobile = commonGatewayObj.Select("SELECT * FROM U_MASTER WHERE LENGTH(MOBILE1) = 11 AND MOBILE1 NOT LIKE '011%' AND  U_MASTER.REG_BK='" + regObj.FundCode.ToString().ToUpper() + "' AND U_MASTER.REG_BR='" + regObj.BranchCode.ToString() + "' AND U_MASTER.REG_NO=" + regObj.RegNumber);
            if(dtValidMobile.Rows.Count>0)
            {
                validMobile = true;
            }
            return validMobile;
